...
The respective SOFT files should be those generated by the
chado2GEO.pl script. geo_lieb will proofread these SOFT files
for errors and fix them accordingly.

=head1 ENVIRONMENT

This script may require access to the modencode-www1 host to copy files
so that their checksum may be calculated. You may want to pre-fetch raw
data files before running this script. geo_build_tarball.pl has a
fetch-only function for this purpose.
...
# The basic idea behind this script is to read in the associated SDRF file
# and identify unique samples. We then determine which files in the submission
# belong to each sample.
#
# We then use the chado2GEO.pl generated SOFT file purely to copy over
# information pertaining to the sample and experimental protocols, removing
# extraneous/duplicate samples and files.
