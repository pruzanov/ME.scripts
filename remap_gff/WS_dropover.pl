...
# Drop over coordinates from GFF, WIG/BED or WIG/variable step from one 
# WS build to an older WS version.
# This is a wrapper for Gary Williams' unmap script, which works.
...
use constant LOCATION => "/home/pruzanov/Data/WormData";
...
while (my $file = shift) {
  ...
  my $outfile = $file;
  if ($outfile =~ /WS\d+/) {
    $outfile =~ s/WS\d+/WS$from/;
  }
  elsif ($outfile =~ /(\.[^\.]+)$/) {
    my $ext = $1;
    $outfile =~ s/$ext$/_WS$from$ext/;
  }
  else {
    $outfile .= "WS$from";
  }
  ...
  print STDERR "Converting $file from WS$to to WS$from.  The outputfile is named $outfile\n";
...
sub convert_gff {
  ...
  system "$remap_path/unmap_gff_between_releases.pl --location=$path --gff=$infile --out=$pwd/tmpout$$ --release1=$from --release2=$to";
