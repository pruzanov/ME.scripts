...
while (my $line = <GFF>) {
  chomp $line;
  if ($line =~ /^\s*$/) {next;}
  if ($line =~ /^\#/) {$line =~ s/$release1/$release2/; print OUT $line,"\n"; next;}
  if ($line =~ /^>/) {print OUT $line,"\n"; while (<GFF>) {print OUT $_;} next;}
  my @f = split /\t/, $line;

      my ($chromosome, $start, $end, $sense) = ($f[0], $f[3], $f[4], $f[6]);
      #print STDERR "chrom, start, end=$chromosome, $start, $end\n";
...
sub remap_gff {
  ...
  if ($chromosome !~ /^(I|II|III|IV|V|X|M)$/) {
    die "Unrecognised chromosome ID: '$chromosome'\nshouldThe chromosome ID must be one of I, II, III, IV, V, X, MtDNA\n";
  }
