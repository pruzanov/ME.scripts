...
my $ssh_key   = "$HOME/cron/pipeline-key"; #.ssh/clinic_key";
my %labs      = (white=>1, 
                 celniker=>1, 
                 lieb=>1, 
                 karpen=>1, 
...
sub check_tracks {
 my $sub = shift @_;
 my @tracks = `ssh -i $ssh_key $pipe_host ls -t /modencode/raw/data/$sub/tracks/ 2> /dev/null`;
 my @extracted = `ssh -i $ssh_key $pipe_host ls -lt /modencode/raw/data/$sub/extracted/`;
 my @data_files = grep {!/^total|README|chadoxml$|idf|sdrf/i} @extracted;
 if (!@tracks || scalar(@tracks) == 0) {
   if (@data_files == 0) {
    print STDERR "For Submission $sub report set to META\n" if DEBUG;
    $reports{$sub}->{report} = "META";
   } else {
...
# Update the database
$dbl->cleanup([keys %delete_these]) unless DEBUG;
